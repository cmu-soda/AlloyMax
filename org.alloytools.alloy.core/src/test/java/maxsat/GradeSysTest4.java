...
run {
	...
	maxsome cs: set Course | all c: cs | maxsome rs: set Role | all r: rs |
...
//        solver.options().setFlatten(false);
        ...
        assertNotNull(set);
        iter = set.iterator();
        assertEquals("[Math$0, Faculty$0]", iter.next().toString());
        assertEquals("[Physics$0, Faculty$0]", iter.next().toString());
        assertEquals("[Physics$0, TA$0]", iter.next().toString());
    }
}
